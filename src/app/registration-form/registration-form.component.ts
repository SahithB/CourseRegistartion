...
export class RegistrationFormComponent implements OnInit {
  ...
  get modeOfPayment() {
    return this.form.get("modeOfPayment");
  }
  get installmentpaid() {
    return this.form.get("installmentpaid");
  }

  onChanges() {
    this.form.get("courseId").valueChanges.subscribe(course => {
      this.courseService.get(course).subscribe(
        c => {
          this.coursechosen = c;
          if (!c.isEMIAllowed) {
            ...
            if (!this.id) this.form.get("modeOfPayment").setValue("Full");
          }
        },
        (error: AppError) => {
    ...
    this.form.get("modeOfPayment").valueChanges.subscribe({
      next: mode => {
        if (mode == "Full") {
  ...
  save() {
    ...
    if (!this.isEMICheck) {
      delete this.formData["installmentpaid"];
    }
    if (this.id)
      this.registrationService.update(this.id, this.formData).subscribe(() => {
        this.router.navigate(["/registrations"]);
      });
    else
      this.registrationService.create(this.form.value).subscribe(() => {
        this.router.navigate(["/registrations"]);
      });
  }
  delete() {
    ...
    this.registrationService
      .delete(this.id)
      .subscribe(() => this.router.navigate(["/registrations"]));
  }
